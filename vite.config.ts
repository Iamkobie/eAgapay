...
// Try to load the Figma Make site config (only exists in Figma Make environment)
let siteConfiguration = {} as any
try {
  siteConfiguration = await import('./.figma/make/site.json')
} catch {
  // Running locally — use empty defaults
}
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    plugins: [
      react(),
      tailwindcss(),
      figmaSiteConfiguration(siteConfiguration?.default ?? siteConfiguration),
